...
struct VsaProperties {
  ...
  Eigen::MatrixXi adjacencies; //MPA //flattened list of adjacency regions with valid edges/midpoints, where MPA(0,0) and MPA(0,1) are the indices of the adjacent regions (useful for looping through edges)
  std::map<int, std::map<int, Eigen::RowVector3d>> edge_centers; //map of 3d midpoints of each region edge, where edgeCenters.at(4).at(1) is a row vector of the x,y,z location of that midpoint
  Eigen::MatrixXd flattened_edge_centers; //flattenedEdgeCenters; //store a #midpoints x 3 matrix of 3d coordinates that can be referenced in the map with vsa_.adjacencies

  std::map<int, std::map<int, int>> edge_center_proxies; //edgeCenterProxies; //map of index in V of nearest vert to the region region edge center
  Eigen::VectorXi proxy_faces; //proxyFaces; //#_region x 1 index of proxy face for each region (the triangle index on the mesh that best approximates the clustered face)
  std::map<int, std::map<int, Eigen::VectorXi>>
      edge_paths;//map of edges per region combination, where edgePaths.at(4).at(1).row(3).col(0) is the index of the first vert in V of the 4th triangle edge on the edge between region 4 and 1
